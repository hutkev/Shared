...
module shared {
  export module router {
     ...
     */
    export class QueueReceiver implements Receiver extends utils.UniqueObject {
    ...
    /* 
     * Reciever that uses basic callbacks
     */
    export class CallbackReceiver implements Receiver, utils.Unique {
      private _address: message.Address;
      private _id;
      private _handle;
      private _fatal;

      constructor (handle: (msg: message.Message) => void ,
        fatal: (fmt: string, ...args: any[]) => void , id?: utils.uid) {
        dassert(utils.isValue(handle));
        dassert(utils.isValue(fatal));
        ...
        this._address = new message.Address(this._id);
      }
